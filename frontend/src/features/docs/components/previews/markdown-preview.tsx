...
customRenderer.heading = ({ text, depth }: { text: string; depth: number }) => {
  ...
  return `<h${depth} id="heading-${slug}" class="scroll-mt-6">${text}</h${depth}>`
}
...
function MarkdownPreviewInner({
  ...
  const html = useMemo(() => {
    if (!deferredContent.trim()) return ''
    const raw = marked.parse(deferredContent) as string
  ...
  const handleHeadingClick = (h: MarkdownHeading) => {
    const container = scrollRef?.current
    if (container) {
      const el = container.querySelector(`#${h.id}`) as HTMLElement | null
      if (el) {
        el.scrollIntoView({ behavior: 'smooth', block: 'start' })
        el.classList.add('bg-primary/10', 'rounded', 'transition-colors', 'duration-500')
        setTimeout(() => {
          el.classList.remove('bg-primary/10')
        }, 1200)
      }
    }
    onNavigateToSource?.(h.text)
  }
  ...
  return (
    <div
      ref={scrollRef}
      onScroll={onScroll}
      className='relative h-full w-full overflow-y-auto p-6 select-text bg-background'
    >
      <div className='relative max-w-5xl mx-auto flex items-start justify-center gap-8 pb-16'>
          ...
          <aside className='sticky top-2 w-48 2xl:w-56 shrink-0 select-none py-1'>
            ...
            <div className='border-l border-border/50 ml-1 pl-2 space-y-0.5 max-h-[calc(100vh-200px)] overflow-y-auto'>
              {headings.length > 0 ? (
                ...
                <p className='text-[11px] text-muted-foreground/60 italic py-1 px-1'>
                  No headings found
                </p>
              )}
            </div>
        ...
        <div
          className='flex-1 max-w-3xl min-w-0 text-foreground text-xs leading-relaxed space-y-3
