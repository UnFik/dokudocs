...
function taskItems(muya: Muya): TaskListItem[] {
  ...
  const visit = (block: {
    ...
    if (
      (block.constructor as { blockName?: string }).blockName ===
      'task-list-item'
    )
...
function checkboxOf(item: TaskListItem): TaskListCheckbox {
  ...
  ;(
    item.attachments as unknown as {
      forEach: (cb: (a: TaskListCheckbox) => void) => void
    }
  ).forEach((a) => {
    if (
      (a.constructor as { blockName?: string }).blockName ===
      'task-list-checkbox'
    )
      found = a
  })
