...
class InlineRenderer {
  ...
  patch(block: Format, cursor?: IRenderCursor, highlights: IHighlight[] = []) {
    ...
    if (
      ...
      typeof this.muya.options.getDecorations === 'function'
    ) {
      const decs = this.muya.options.getDecorations(block)
      if (decs && decs.length) effectiveHighlights = decs
    }

    const tokens = this._tokenizer(block, effectiveHighlights)
