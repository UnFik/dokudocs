...
describe('markdownToState — task list nesting (marktext 23435ce6)', () => {
  it('keeps an empty unchecked task item after a populated task item', () => {
    ...
    expect(list.children!.map((c) => c.name)).toEqual([
      'task-list-item',
      'task-list-item',
    ])
    expect(list.children!.map((c) => c.meta?.checked)).toEqual([false, false])
    expect(
      list.children![0].children!.find((c) => c.name === 'paragraph')?.text
    ).toBe('a')
    expect(list.children![1].children).toEqual([
      { name: 'paragraph', text: '' },
    ])
  })
  ...
  it('parses a single empty unchecked task item as a task list item', () => {
    ...
    expect(states[0].children![0].children).toEqual([
      { name: 'paragraph', text: '' },
    ])
  })
  ...
  it('parses an empty task marker with lazy continuation text as a task item', () => {
    ...
    expect(states[0].children![0].children![0]).toEqual({
      name: 'paragraph',
      text: 'text',
    })
  })
  ...
  it('keeps lazy continuation text on the final empty task marker in a task list', () => {
    ...
    expect(states[0].children![1].children).toEqual([
      { name: 'paragraph', text: '' },
    ])
    expect(states[0].children![2].children).toEqual([
      { name: 'paragraph', text: '' },
    ])
    expect(states[0].children![3].children![0]).toEqual({
      name: 'paragraph',
      text: 'text',
    })
  })
...
`
    ...
    expect(level3.children!.find((c) => c.name === 'paragraph')?.text).toBe(
      'task1_1_1'
    )
  })
...
`
    ...
    expect(
      level2Nested,
      'level 2 should contain level 3 nested, not as sibling'
    ).toBeDefined()
  })
})
...
describe('markdownToState — trimUnnecessaryCodeBlockEmptyLines (#1265)', () => {
  ...
  it('honours the option through a stateToMarkdown round-trip', () => {
    ...
    const trimmedStates = generate(fenced, {
      trimUnnecessaryCodeBlockEmptyLines: true,
    })
    ...
    const keptStates = generate(fenced, {
      trimUnnecessaryCodeBlockEmptyLines: false,
    })
