...
function canPlainUrlFallbackAutoLink(
  ...
  const candidate =
    content.substring(0, start.offset) + text + content.substring(end.offset)
...
function shouldPreserveBareUrlLinkForPaste(
  ...
  return (
    ...
    !canPlainUrlFallbackAutoLink(text, content, start, end)
  )
}
...
function inlineMergeText(
  state: TState,
  anchorHasText: boolean
): Nullable<string> {
  switch (state.name) {
    case 'paragraph':
      return state.text
...
function tryMergeListPaste(
  ...
  const paraIndex = ctx.wrapperBlock
    ? (listItem as Parent).offset(ctx.wrapperBlock)
    : 0
  ...
  const canFold =
    firstState.name !== 'task-list' && isParagraphState(pastedFirst)
  ...
  if (canFold) {
    ...
    currentItem.children = [
      ...currentItem.children,
      ...pastedItems[0].children.slice(1),
    ]
...
function applyPlainTextBlockHtml(
  clipboard: Clipboard,
  ctx: IPasteContext,
  text: string
): void {
  ...
  const newBlock = ScrollPage.loadBlock(htmlState.name).create(
    clipboard.muya,
    htmlState
  )
