...
function selectedTableCells(
  ...
  const selectedDoms = domNode.querySelectorAll(
    `.${CLASS_NAMES.MU_TABLE_CELL_SELECTED}`
  )
...
export function cutSelection(clipboard: Clipboard): void {
  ...
  if (isSelectionInSameBlock) {
    ...
    const endOffset =
      direction === SelectionDirection.FORWARD ? focus.offset : anchor.offset
  ...
  const startBlock =
    direction === SelectionDirection.FORWARD ? anchorBlock : focusBlock
  const endBlock =
    direction === SelectionDirection.FORWARD ? focusBlock : anchorBlock
  const startOffset =
    direction === SelectionDirection.FORWARD ? anchor.offset : focus.offset
  const endOffset =
    direction === SelectionDirection.FORWARD ? focus.offset : anchor.offset
...
function collapseLanguageInputCut(
  ...
  paragraph
    ...
    ?.setCursor(startOffset, startOffset, true)
