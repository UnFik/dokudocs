...
class Clipboard {
  ...
  private _listen() {
    ...
    const copyCutHandler = (event: Event) => {
      ...
      if (isCut && !this.muya.options.readOnly) this.cutHandler()
    }

    const keydownHandler = (event: Event) => {
      if (!ownsEvent() || !isKeyboardEvent(event) || this.muya.options.readOnly)
        return
    ...
    const pasteHandler = (event: Event) => {
      if (ownsEvent() && isClipboardEvent(event) && !this.muya.options.readOnly)
