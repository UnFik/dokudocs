...
abstract class BaseScrollFloat extends BaseFloat {
    ...
    protected activeEleScrollIntoView(ele: HTMLElement) {
        if (!ele) return;
        const scrollParent = this.container || this.scrollElement;
        if (!scrollParent) return;

        const parentRect = scrollParent.getBoundingClientRect();
        const eleRect = ele.getBoundingClientRect();

        if (eleRect.top < parentRect.top) {
            scrollParent.scrollTop -= (parentRect.top - eleRect.top);
        } else if (eleRect.bottom > parentRect.bottom) {
            scrollParent.scrollTop += (eleRect.bottom - parentRect.bottom);
        }
    }
