...
function asRenderer(
  r:
    | IFakeRenderer
    | { loadImageMap: Map<string, unknown>; urlMap: Map<string, string> }
): Renderer {
...
interface IFakeRenderer {
  loadImageMap: Map<
    string,
    { id: string; isSuccess: boolean; width?: number; height?: number }
  >
...
describe('loadImageAsync — small image class on first load', () => {
  ...
  async function runLoad(loadResult: {
    url: string
    width: number
    height: number
  }) {
