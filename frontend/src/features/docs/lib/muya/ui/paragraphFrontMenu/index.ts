...
export class ParagraphFrontMenu extends BaseFloat {
  ...
  private _turnIntoBlock(label: string, block: Parent, oldState: TState) {
    ...
    switch (block.blockName) {
      ...
      case 'atx-heading': {
        ...
        const headingLevel = isAtxHeadingState(oldState)
          ? oldState.meta.level
          : null
        if (
  ...
  private _turnIntoList(label: string, block: Parent, oldState: TState) {
    ...
    const delimiter =
      'delimiter' in sourceMeta ? sourceMeta.delimiter : orderListDelimiter
    const marker = 'marker' in sourceMeta ? sourceMeta.marker : bulletListMarker

    const childContents: TState[][] = oldState.children.map((li) =>
