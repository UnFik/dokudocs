...
class TreeNode implements ILinkedNode {
     ...
     */
    createDomNode() {
        const { tagName, classList, attributes, datasets, muya } = this;

        const effectiveAttributes = attributes ? { ...attributes } : {};
        if (muya?.options?.readOnly && 'contenteditable' in effectiveAttributes) {
            effectiveAttributes.contenteditable = false;
        }
