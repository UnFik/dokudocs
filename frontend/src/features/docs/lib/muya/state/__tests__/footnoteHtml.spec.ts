...
describe('renderToStaticHTML — footnote backref list (PR-8c)', () => {
  ...
  it('numbers inline references in source order, regardless of definition order', () => {
    ...
    expect(html).toMatch(
      /A<sup class="footnote-ref"><a href="#fn-1"[^>]*>1<\/a><\/sup>/
    )
    expect(html).toMatch(
      /B<sup class="footnote-ref"><a href="#fn-2"[^>]*>2<\/a><\/sup>/
    )
  ...
  it('points every repeated inline reference to the same `#fn-N` target', () => {
    ...
    const refs =
      html.match(
        /<sup class="footnote-ref"><a href="#fn-\d+"[^>]*>\d+<\/a><\/sup>/g
      ) ?? []
  ...
  it('preserves a footnote definition that contains a nested bullet list inside the <li>', () => {
    ...
    const sectionMatch = html.match(
      /<section class="footnotes">[\s\S]*<\/section>/
    )
    expect(
      sectionMatch,
      'expected a footnotes <section> to be present'
    ).not.toBeNull()
