...
const CROSS_BLOCK_LIST_LABELS = new Set([
  'bullet-list',
  'order-list',
  'task-list',
])
...
export class Muya {
  ...
  init() {
    ...
    this.eventCenter.on('json-change', () => {
      this.eventCenter.emit('change', { markdown: this.getMarkdown() })
    })
  ...
  setReadOnly(readOnly: boolean) {
    ...
    if (this.domNode) {
      ...
      const editables = this.domNode.querySelectorAll<HTMLElement>(
        '.mu-content, .mu-language-input, [contenteditable]'
      )
      for (let i = 0; i < editables.length; i++) {
        editables[i].setAttribute(
          'contenteditable',
          readOnly ? 'false' : 'true'
        )
      }
  ...
  forceUpdateDecorations() {
    const { scrollPage, selection } = this.editor
    ...
    if (currentSel && selection) {
      try {
        selection.setSelection(currentSel.anchor, currentSel.focus)
      } catch (err) {
        void err
      }
    }
  ...
  replaceContent(
    ...
    const boundarySelection =
      recordSelection !== undefined ? recordSelection : selection
  ...
  setOptions(options: Partial<IMuyaOptions>, forceRender = false) {
    ...
    if ('spellcheckEnabled' in options)
      this.domNode.setAttribute(
        'spellcheck',
        options.spellcheckEnabled ? 'true' : 'false'
      )
  ...
  private _forceRender() {
    ...
    if (selection && selection.isSelectionInSameBlock) {
      ...
      const cursorBlock = this.editor.scrollPage?.queryBlock(
  ...
  private _orderedSelectionRange() {
    ...
    return {
      ...
      firstOffset:
        (forward ? selection.anchor?.offset : selection.focus?.offset) ?? 0,
      lastOffset:
        (forward ? selection.focus?.offset : selection.anchor?.offset) ?? 0,
    }
  }
  ...
  hasFocus() {
    ...
    if (
      ...
      (this.domNode === selection.anchorNode ||
        this.domNode.contains(selection.anchorNode))
    ) {
      return true
    }
  ...
  private _handleCrossBlockParagraph(type: string): boolean {
    ...
    if (CROSS_BLOCK_LIST_LABELS.has(label)) {
      this._wrapSelectedBlocksInList(
        label as 'bullet-list' | 'order-list' | 'task-list'
      )
      return true
    }
  ...
  private _selectionEndpoints(): { anchor: Parent; focus: Parent } | null {
    ...
    const live = endpointPair(
      sel?.anchor.block?.outMostBlock,
      sel?.focus.block?.outMostBlock
    )
  ...
  private _wrapSelectedBlocks(
    ...
    const container = ScrollPage.loadBlock(state.name).create(
      this,
      state as never
    )
  ...
  private _wrapSelectedBlocksInList(
    label: 'bullet-list' | 'order-list' | 'task-list'
  ) {
    const { bulletListMarker, orderListDelimiter, preferLooseListItem } =
      this.options
  ...
  createTable(
    ...
    const safeColumns = Math.max(
      1,
      Number.isFinite(columns) ? Math.floor(columns) : 0
    )
  ...
  insertImage({ src = '', alt = '' }: { src?: string; alt?: string }) {
    ...
    block.text =
      text.substring(0, start.offset) + imageText + text.substring(end.offset)
  ...
  private _withPreservedOffset(fn: () => void) {
    ...
    const multiBlock =
      !!anchorBlock && !!focusBlock && anchorBlock !== focusBlock
    ...
    if (multiBlock && anchorText != null && focusText != null) {
      ...
      if (a && f) {
        this.editor.selection.setSelection(
          {
            offset: clampTo(anchorOffset, a.text.length),
            block: a,
            path: a.path,
          },
          {
            offset: clampTo(focusOffset, f.text.length),
            block: f,
            path: f.path,
          }
        )
  ...
  private _findContentByText(text: string): Content | null {
    let leaf: Nullable<Content> =
      this.editor.scrollPage?.firstContentInDescendant()
    while (leaf) {
  ...
  private _unwrapToParagraphs(block: Parent) {
    ...
    for (const childState of inner) {
      const newBlock = ScrollPage.loadBlock(childState.name).create(
        this,
        childState
      )
  ...
  private _changeHeadingLevel(
    block: Parent,
    type: 'upgrade heading' | 'degrade heading'
  ) {
    const state = block.getState()
  ...
  private _snapshotSelection(): ISelectionSnapshot | null {
    ...
    const cachedCrossBlock =
      !!sel.anchorBlock &&
      !!sel.focusBlock &&
      sel.anchorBlock !== sel.focusBlock
  ...
  private _restoreSelection(snapshot: ISelectionSnapshot | null): boolean {
    ...
    this.editor.selection.setSelection(
      {
        offset: snapshot.anchor,
        block: anchorBlock,
        path: [...snapshot.anchorPath],
      },
      {
        offset: snapshot.focus,
        block: focusBlock,
        path: [...snapshot.focusPath],
      }
    )
  ...
  private _convertListType(block: Parent, label: string) {
    ...
    const childContents: TState[][] = state.children.map((li) =>
      deepClone(li.children)
    )
...
function getContainer(originContainer: HTMLElement, options: IMuyaOptions) {
  ...
  if (!hideQuickInsertHint)
    originContainer.classList.add(CLASS_NAMES.MU_SHOW_QUICK_INSERT_HINT)

  if (spellcheckHideMarks)
    originContainer.classList.add(CLASS_NAMES.MU_HIDE_SPELLING_MARKS)

  // Apply focus mode at construction when initially enabled; `setFocusMode`
  // toggles it thereafter.
  if (focusMode) originContainer.classList.add(CLASS_NAMES.MU_FOCUS_MODE)

  originContainer.classList.add(CLASS_NAMES.MU_EDITOR)
  if (options.readOnly) {
    originContainer.classList.add('mu-read-only')
  }

  originContainer.setAttribute(
    'contenteditable',
    options.readOnly ? 'false' : 'true'
  )
  originContainer.setAttribute('autocorrect', 'false')
  originContainer.setAttribute('autocomplete', 'off')
  originContainer.setAttribute(
    'spellcheck',
    spellcheckEnabled ? 'true' : 'false'
  )

  applyAppearance(originContainer, options)
