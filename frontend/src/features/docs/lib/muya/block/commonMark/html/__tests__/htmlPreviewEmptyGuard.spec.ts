...
describe('isEmptyHtmlBlock (#3821)', () => {
  ...
  it('does not treat self-contained media (video/audio) as empty', () => {
    expect(
      isEmptyHtmlBlock('<video controls src="https://e.com/v.mp4"></video>')
    ).toBe(false)
    expect(
      isEmptyHtmlBlock('<video src="https://e.com/v.mp4">\n</video>')
    ).toBe(false)
    expect(isEmptyHtmlBlock('<audio src="https://e.com/a.mp3"></audio>')).toBe(
      false
    )
  })
