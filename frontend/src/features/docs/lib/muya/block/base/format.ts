...
class Format extends Content {
  ...
  unlink({
    ...
    const oldText = this.text
    this.text =
      oldText.substring(0, range.start) + text + oldText.substring(range.end)
    ...
    if (event.ctrlKey || (event.altKey && !event.shiftKey)) {
      ...
      if (start.offset < text.length) {
        ...
        if (leadingSpaces) {
          ...
          deleteLen =
            leadingSpaces[0].length + (wordMatch ? wordMatch[0].length : 0)
        } else {
        ...
        const nextOffset = Math.min(
          text.length,
          start.offset + Math.max(1, deleteLen)
        )
      ...
      if (type === 'image') {
        ...
        requestAnimationFrame(() => {
          ...
          if (isHTMLElement(startNode)) {
            const imageWrapper =
              startNode.closest<HTMLElement>('.mu-inline-image')
  ...
  private _emitFootnoteToolEvent(reference: HTMLElement) {
    ...
    scrollPage.breadthFirstTraverse((node) => {
      ...
      const id = (node as TreeNode & { meta?: { identifier?: string } }).meta
        ?.identifier
