...
export function getIcon(block: Parent) {
    ...
    switch (blockName) {
        ...
        case 'atx-heading':
            return HEADING_ICONS[(block as AtxHeading).meta.level - 1] || lucideIcons.heading1;

        case 'setext-heading':
            return HEADING_ICONS[(block as SetextHeading).meta.level - 1] || lucideIcons.heading1;
        ...
        case 'diagram':
            return DIAGRAM_ICONS[(block as DiagramBlock).meta.type] || lucideIcons.diagram;
