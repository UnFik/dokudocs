...
export class Muya {
    ...
    setReadOnly(readOnly: boolean) {
        ...
        if (this.domNode) {
            ...
            const editables = this.domNode.querySelectorAll<HTMLElement>('.mu-content, .mu-language-input, [contenteditable]');
            for (let i = 0; i < editables.length; i++) {
                editables[i].setAttribute('contenteditable', readOnly ? 'false' : 'true');
            }
        }
        if (readOnly && this.ui) {
...
function getContainer(originContainer: HTMLElement, options: IMuyaOptions) {
    ...
    if (focusMode)
        originContainer.classList.add(CLASS_NAMES.MU_FOCUS_MODE);
    ...
    originContainer.setAttribute('contenteditable', options.readOnly ? 'false' : 'true');
    originContainer.setAttribute('autocorrect', 'false');
    originContainer.setAttribute('autocomplete', 'off');
    originContainer.setAttribute('spellcheck', spellcheckEnabled ? 'true' : 'false');
