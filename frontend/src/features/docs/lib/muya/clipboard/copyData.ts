...
function resolveSelectionOrder(
  ...
  const anchorOutMostBlockOffset =
    clipboard.scrollPage?.offset(anchorOutMostBlock)
  const focusOutMostBlockOffset =
    clipboard.scrollPage?.offset(focusOutMostBlock)
...
function buildRangeContainerState(
  ...
  const startChildFull = childStates[startIdx] as TState & {
    children?: TState[]
  }
  const startState =
  ...
  return {
    ...
    children: [
      startState,
      ...childStates.slice(startIdx + 1, endIdx),
      endState,
    ],
  } as TState
}
...
function appendPartialState(
  ...
  if (
    /block-quote|bullet-list|order-list|task-list/.test(outBlock!.blockName)
  ) {
    copyState.push(
...
function collectSameOutMostBlockState(order: ICopyOrder): TState[] {
  const { anchorOutMostBlock, startBlock, endBlock, startOffset, endOffset } =
    order
...
function collectCopyState(order: ICopyOrder): TState[] {
  const { anchorOutMostBlock, focusOutMostBlock, startOutBlock, endOutBlock } =
    order
